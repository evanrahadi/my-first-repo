...
if((isset($_GET['bulan']) && $_GET['bulan']!='') && (isset($_GET['tahun']) && $_GET['tahun'] != '')) {
	...
	$sql =mysqli_query($konek, "SELECT pegawai.nip,pegawai.nama_pegawai,jabatan.nama_jabatan,golongan.nama_golongan,master_gaji.potongan FROM pegawai INNER JOIN jabatan ON jabatan.kode_jabatan=pegawai.kode_jabatan INNER JOIN golongan ON golongan.kode_golongan=pegawai.kode_golongan INNER JOIN master_gaji ON master_gaji.nip=pegawai.nip WHERE master_gaji.bulan='$bulantahun' ORDER BY pegawai.nip ASC");
...
if(mysqli_num_rows($sql) < 1){
	echo "<tr> <tdcolspan='6'> Belum Ada data... </td> </tr>";
}
?>
</table>
<table width="100%">
