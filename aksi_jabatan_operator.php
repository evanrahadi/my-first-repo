	...
	if($_GET['act']=='insert'){
		...
		$tunj  = $_POST['tunjanganjabatan'];
			...
			if($kode=='' || $nama=='' || $gapok=='' || $tunj=='') {
				header('location:data_jabatan_operator.php?view=tambah&e=bl');
			}else{
				...
				if($simpan){
					header('location:data_jabatan_operator.php?e=sukses');
				}else{
					header('location:data_jabatan_operator.php?e=gagal');
				}
			}
	}
